...
void writeEncodedData(FILE* src, FILE* dest, HuffCode* huffCodeTable, unsigned int fileSize)
{
    ...
    while ((c = fgetc(src)) != EOF)
    {
        ...
        {
            cancel = true;
            Common::showDoneMessage("Отменено!", "Сжатие данных");
            break;
        }
    }
...
void writeDecodedData(FILE * src, FILE * dest, HuffNode * rootTree, unsigned int fileSize)
{
    ...
    while (bytesWritten < fileSize)
    {
        ...
        {
            cancel = true;
            Common::showDoneMessage("Омененно!", "Восстановление данных");
            break;
        }
    }
...
void huffmanEncode(const char* inputFile, const char* outputFile)
{
    ...
    // Посчитать количество частотностей (т.е. тех которые больше нуля):
